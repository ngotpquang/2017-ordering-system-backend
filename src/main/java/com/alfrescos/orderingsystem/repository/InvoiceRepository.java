...
public interface InvoiceRepository extends CrudRepository<Invoice, String>{
    @Query(value = "SELECT * FROM invoice WHERE customer_user_id = ?1 AND is_visible = 1 AND is_paid = 1", nativeQuery = true)
...
//    @Query(value = "SELECT * FROM invoice WHERE staff_user_id = ?1 AND is_visible = TRUE AND is_paid = TRUE", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM invoice WHERE paying_time IS NULL AND is_visible = 1 ORDER BY created_date DESC", nativeQuery = true)
//    @Query(value = "SELECT * FROM invoice WHERE paying_time IS NULL AND is_visible = TRUE ORDER BY created_date DESC", nativeQuery = true)
...
//    @Query(value = "SELECT * FROM invoice WHERE paying_time BETWEEN TO_TIMESTAMP(?1 ,'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP(?2 ,'YYYY-MM-DD HH24:MI:SS') AND is_paid = TRUE", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM invoice WHERE is_paid = 1 ORDER BY paying_time ASC", nativeQuery = true)
