...
public interface InvoiceRepository extends CrudRepository<Invoice, String>{
    ...
    @Query(value = "SELECT * FROM invoice WHERE CONVERT(VARCHAR(25), paying_time, 126) LIKE ?1%", nativeQuery = true)
    public List<Invoice> findAllInvoicesByDate(String date);

    @Query(value = "SELECT * FROM invoice WHERE paying_time >= ?1 AND paying_time <= ?2", nativeQuery = true)
